...
_DEFAULT_CAPTURE_DIR = Path("results/visual")
...
def _resolve_llm_provider(model_name: str) -> str:
    if not model_name:
        return model_name
    if "/" in model_name:
        return model_name
...
def _build_llm_extraction_strategy(*, instruction: str) -> LLMExtractionStrategy:
    ...
    llm_config = LLMConfig(
        provider=_resolve_llm_provider(config.info_llm_model_name),
        api_token=config.info_llm_model_api_key,
...
_UMA_LLME_INSTRUCTION = (
    "Extract only the character page content for Uma Musume. "
    "Focus on: basic profile/infobox fields, introduction, personality, story, "
    "relationships, original horse notes, and other factual sections. "
    "Ignore navigation, UI chrome, edit controls, ads, scripts, styles, "
    "video placeholders, and repeated boilerplate."
)
        ...
        if structured:
            ...
            if not html_ready and allow_render_fallback:
                ...
                if render_url and render_url != crawl_url:
                    render_result = await crawler.arun(
                        url=render_url,
                        config=_build_run_config(
                            use_proxy,
                            css_selector=css_selector,
...
async def _crawl_page_visual(
    ...
    *,
    ...
    crawl_url = target_url or url
    source_url = source_url or url
    output_dir = output_dir or _DEFAULT_CAPTURE_DIR
...
async def crawl_page(url: str, *, use_proxy: bool = False) -> str:
    return await _crawl_with_config(
        url,
        target_url=None,
        source_url=None,
        use_proxy=use_proxy,
...
async def crawl_biligame_page(url: str, *, use_proxy: bool = False) -> str:
    return await _crawl_with_config(
        url,
        target_url=None,
        source_url=None,
...
async def crawl_moegirl_page(url: str, *, use_proxy: bool = True) -> str:
...
async def crawl_biligame_page_pruned(url: str, *, use_proxy: bool = False) -> str:
    return await _crawl_with_config(
        url,
        target_url=None,
        source_url=None,
...
async def crawl_moegirl_page_pruned(url: str, *, use_proxy: bool = True) -> str:
    render_url = _build_moegirl_render_url(url) or url
    return await _crawl_with_config(
        url,
        target_url=render_url,
...
async def crawl_biligame_page_llm(url: str, *, use_proxy: bool = False) -> str:
    return await _crawl_with_config(
        url,
        target_url=None,
        source_url=None,
        ...
        allow_render_fallback=False,
        markdown_generator=_build_pruning_markdown_generator(),
        extraction_strategy=_build_llm_extraction_strategy(
            instruction=_UMA_LLME_INSTRUCTION
        ),
        prefer_fit_markdown=False,
...
async def crawl_moegirl_page_llm(url: str, *, use_proxy: bool = True) -> str:
    render_url = _build_moegirl_render_url(url) or url
    return await _crawl_with_config(
        url,
        target_url=render_url,
...
async def crawl_biligame_page_visual_docling(
    ...
    *,
    ...
    return convert_docling(target_path)


async def crawl_moegirl_page_visual_docling(
    url: str,
    *,
    use_proxy: bool = True,
    output_dir: Path | None = None,
...
async def crawl_page_visual_markitdown(
    ...
    *,
    ...
    return convert_markitdown(pdf_path, use_llm=False)
